...
//Placement de toutes les pièces sur l'échiquier
...
{
    //Paramétrage des couleurs des interactions ImGui
    ...
    //On affiche l'échiquier en bouclant sur les cases et on créé un tableau de pointeurs pour les pièces
    ...
    //On gère les interactions de la souris
    if (ImGui::IsMouseClicked(ImGuiMouseButton_Right))
    {
        m_selectedPieceIndex = -1;
    }

    //On affiche les cases de l'échiquier
    for (int x = 0; x < 8; x++)
    {
        for (int y = 0; y < 8; y++)
        {
            //On utilise un ID unique pour chaque case pour éviter les conflits d'ID
            int index = x * 8 + y;
            ImGui::PushID(index);

            //On utilise notre fonction des différents mouvements possibles de chaque pièce
            ...
            //Si notre case contient une pièce, on utilise la fonction get_type() pour afficher le type de la pièce
            //+ on utilise la fonction get_color() pour afficher la couleur de la pièce
            if (m_board[index])
            {
            ...
            //Tant que le jeu n'est pas terminé, on continue d'afficher les pièces et notre échiquier
            if (!m_gameOver && ImGui::Button(label.c_str(), ImVec2{70.f, 70.f}))
            {
                ...
                else
                {
                    if (std::find(possibleMoves.begin(), possibleMoves.end(), index) != possibleMoves.end())
                    {
                        //On gère la fin de la partie par un message en prenant en compte l'index du roi
                        ...
                        // On vérifie si le pion a atteint la dernière rangée
                        if (m_board[m_selectedPieceIndex]->get_type() == PieceType::Pawn)
                        {
                            int row = index / 8;
                            if ((m_board[m_selectedPieceIndex]->get_color() == Color::White && row == 0) || (m_board[m_selectedPieceIndex]->get_color() == Color::Black && row == 7))
                            {
                                isPromotion = true;
                            }
                        }
                        // Si le mouvement est une promotion, on affiche la popup de promotion
                        if (isPromotion)
                        {
                        ...
                        // Sinon, on effectue le mouvement classique
                        else
                        {
    ...
    //On utilise une popup modal de ImGui pour afficher le message de victoire
    ...
    //On utilise une popup modal de ImGui pour afficher le message de promotion
    if (ImGui::BeginPopupModal("Promotion", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        //On récupère via un switch le type de pièce à promouvoir
        ...
        //On affiche les boutons pour choisir la pièce de promotion 
